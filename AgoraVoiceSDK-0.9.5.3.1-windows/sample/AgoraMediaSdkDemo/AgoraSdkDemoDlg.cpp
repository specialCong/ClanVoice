...
void CAgoraSdkDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_VIDEO_LARGE, m_remoteView);
	DDX_Control(pDX, IDC_VIDEO_SMALL, m_localView);
	DDX_Check(pDX, IDC_CHECK_ENABLE_VIDEO, m_bEnableVideo);
}
...
void CAgoraSdkDemoDlg::OnBnClickedBtnJoin()
{
	...
	if (!m_pAgoraAudio)
	{
		...
		// Setup Video context
		chat_engine_context_t ctx;
		if (m_bEnableVideo)
		{
			ctx.remote.view = m_remoteView.GetSafeHwnd();
			ctx.local.view = m_localView.GetSafeHwnd();
		}
		else
		{
			ctx.remote.view = NULL;
			ctx.local.view = NULL;
		}
		m_pAgoraAudio->setAppContext(&ctx, sizeof(ctx));
